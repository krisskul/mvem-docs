# Script to autogenerate .rst files for autodocumentation of classes and modules in mvem.
...
.. GENERATED BY `generate_module_rst.py`
.. DATE: {date}
{content}
"""
...
def is_documentable_module(m: Any) -> bool:
    ...
    if not hasattr(m, "__file__"):
        return False
    stem = m.__file__.split("\\lib")[-1]
    return inspect.ismodule(m) and ("mvem" in m.__name__ or "mvem" in stem) and m.__name__ not in IGNORE_MODULES

def is_documentable_component(m: Any, nodename: str) -> bool:
    """Return `True` if a function or class to be documented automatically, `False` otherwise.
    """
    if inspect.isfunction(m):
        return nodename in m.__module__ and m.__module__ not in IGNORE_MODULES
...
def get_component_rst_string(module_name: str, component: Callable, level: int) -> str:
    ...
    """
    object_name = f"{module_name}.{component.__name__}"
...
def get_module_rst_string(level: int, modulename: str) -> str:
    """Get an rst string, used to autogenerate documentation for a module
    :param modulename: the module name
    :param level: the level in nested directory structure
    """
    level_underline = RST_LEVEL_SYMBOLS[level] * len(modulename)
    return SPHINX_INCLUDE_MODULE_STRING.format(
        name=modulename, module=modulename.split(".")[-1], level=level_underline
    )
...
def traverse_module_bfs(queue: List[Tuple[Any, int]], enqueued_items: Set[int]):
    ...
    while queue:
        ...
        if not hasattr(node, "__name__"): continue
        if is_documentable_module(node):

            # find module name
            if hasattr(node, "__path__"):
                stem = node.__path__[0]
            elif hasattr(node, "__file__"):
                stem = node.__file__.split(".py")[0]
            else:
                raise Exception("skip")

            stem = stem.split("\\lib")[-1].split("mvem")[-1].split("\\")
            stem = list(filter(lambda d: len(d)>0, stem))
            stem = ["mvem"] + stem
            ...
            for child in get_public_attributes(node):
                ...
                if is_documentable_component(child, node.__name__):
                    # pass in correct name ....
                    modulename = ".".join(stem)
                    rst_components.append(get_component_rst_string(modulename, child, level))
                    enqueued_items.add(id(child))

                elif is_documentable_module(child):

                    if "." in child.__name__:
                        continue

                    if do_visit_module(child, enqueued_items):

                        modulename = ".".join(stem + [child.__name__])

                        mod = get_module_rst_string(level, modulename)
                        rst_modules.append(mod)
            ...
            if rst_content:
                nodename = ".".join(stem)
                write_to_rst_file(nodename, rst_content)


if __name__ == "__main__":
    set_global_path(os.path.dirname(os.path.realpath(__file__)))
    traverse_module_bfs([(mvem, 0)], set([id(mvem)]))
